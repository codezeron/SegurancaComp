...
def autentication(chave_privada):
    ...
    dados_criptografados_io.write(mensagem_claro + assinatura_ec + len(assinatura_ec).to_bytes(1,byteorder='big'))
...
def verification(chave_publica):
    ...
    with open('dados.tar', 'rb') as arquivo_tar:
        ...
        tam = int.from_bytes(dados_criptografados[-1:], byteorder='big') + 1
        
        # tirar o hash dos dados criptografados
        hash_extraido = dados_criptografados[-tam:-1]
        
        #tirar o texto claro dos dados criptografados
        texto_claro = dados_criptografados[:-tam]
...
autentication(chave_privada_ec)
verification(chave_publica_ec)